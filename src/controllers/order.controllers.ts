...
const createOrderCheckout = catchAsync(
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    ...
    if (!bookIds || !user || !price)
      return next(new AppError(`Order creation failed`, HTTP_STATUS.BAD_REQUEST));
    // Check if the user in the query parameters matches the authenticated user
    if (user !== req.user.id) {
      return next(
        new AppError('User identity mismatch. Order creation failed.', HTTP_STATUS.FORBIDDEN)
      );
    }
